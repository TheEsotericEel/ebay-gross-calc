(() => {
  // prevent duplicate mounts on SPA navigations
  // @ts-ignore
  if ((window as any).__EGC_LOADED__) { console.log("[EGC] already loaded"); return; }
  // @ts-ignore
  ...
  try {
    ...
    // --- Local math (no imports) ---
    type Order = { gross: number; itemPrice?: number; shippingCost: number; cogs: number; fees?: number; ads?: number; refunds?: number; };
    function estimateFees(o: Order, feeRate = 0.13, procRate = 0.029, procFixed = 0.30) {
      const base = o.itemPrice ?? o.gross;
      return base * feeRate + base * procRate + procFixed;
    }
    function computeKPIs(orders: Order[], weeklySalesGoal: number, listingGoalDaily: number, listedToday: number) {
...
$("openOptions").addEventListener("click", () => {
  ...
  window.open(url, "_blank");
    ...
    $("saveListed").addEventListener("click", async () => {
      ...
      const k = computeKPIs((orders as Order[]), Number(weeklyGoal)||0, Number(listingGoalDaily)||0, Number(listedToday)||0);
